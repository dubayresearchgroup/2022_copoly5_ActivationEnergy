...
class Polymer:
    ...
    def countAB(cls, atomtypes, conf):
        """Takes mapping of atom ID to atom type along with polymer configuration dictionary (conf) and create a collections.Counter object from the unique naming of each monomer.
    ...
    def chainlength(self):
        ...
        for chain in self.seqs:
            ...
            blockAerr += list(map(len, re.findall('(A+){1,4}',chain))) #RLH
            blockBerr += list(map(len, re.findall('(B+){1,4}',chain))) #RLH
            blockAerr += list(map(len, regex.findall('(AAAAAA+){s<=1}',chain))) #RLH
            blockBerr += list(map(len, regex.findall('(BBBBBB+){s<=1}',chain))) #RLH
        ...
        self.blockAerror = list(zip(*sorted(list(Counter(blockAerr).items())))) #RLH
        self.blockBerror = list(zip(*sorted(list(Counter(blockBerr).items())))) #RLH
    ...
    def chainends(self):
        """Identifies the central monomer atom of the chain ends of all chains. Note this doesn't destinguish closed-loop chains in the flexible chain case."""
        self.chainends = []
        for chain in self.chains:
            ends = [chain[1],chain[-2]]
            self.chainends.append(ends)
    ...
    def getrecord(self):
        ...
        return {
            ...
            "PDI": self.pdi,
            "seq": self.seqlist,
            "numseq": self.chains, #RL
            "chainends": self.chainends,  #RLH
            ...
            "blockAerror": self.blockAerror, #RLH
            "blockBerror": self.blockBerror  #RLH
        }
